...
parser=argparse.ArgumentParser(description = 'Welcome to the Benchmarking set of Universal Single Copy Orthologs (BUSCO).\n\n For further usage information, please check the README file provided with this distrubution.',
                               usage = 'BUSCO_v2.0.py -in [SEQUENCE_FILE] -l [LINEAGE] -o [OUTPUT_NAME] [OTHER OPTIONS]')
...
parser.add_argument('-lim', '--limit', metavar = 'region_limit', default = 3, type = int, help = 'How many candidate regions to consider (default: 3)')
...
if mode == 'genome' or mode == 'blast':  
  ...
  for busco_group in coords:
      ...
      for contig in candidate_contigs:
          ...
          ###DBG BLOCK            
#          if busco_group == 'BUSCOaEOG7G7Z35' and contig == 'JH431661':
...
#          if busco_group == 'BUSCOaEOG7G7Z35' and contig == 'JH431661':
#              print(final_regions)  
#              print(gargantua(final_regions))
      
      ###DBG BLOCK 2
#      if busco_group == 'BUSCOaEOG7G7Z35':
#          print(size_lists)
...
if mode == 'transcriptome' or mode == 'trans':
  ...
  for entry in lista:
    ...
    trans_seq.close()
    #subprocess.call('python six.py %s %s' % (args['tmp'] + entry, mainout + 'translated_proteins/' + entry.split(args['abrev'])[0] + '.fas'), shell = True)  
...
if mode == 'genome': 
  ...
  if os.path.exists(mainout+'augustus_proteins') == False:
    subprocess.call('mkdir %saugustus_proteins' % mainout, shell = True)
...
if mode == 'genome':
    subprocess.call('find %saugustus_proteins -size  0 -print0 |xargs -0 rm' % mainout, shell = True)
...
def parse_hmmer(hmmer_results_files, mainout, mode, location_dic=False):
    ...
    ##REFINE CLASSIFICATION (ALL FILES ARE PARSED)
    #print('Total number of BUSCO evaluated: %s' % len(everything.keys()))      
...
#DBG
#            sigma = everything[thing][sequence][0][-1]
#            print(all_data)
#            a = input('continue...\n')
    ...
    #print('Total number of BUSCO that are complete (SC+MC): %s' % len(is_complete.keys()))    
...
#DBG
#print('**DBG**')
#print(len(complete_and_singlecopy))
#print(complete_and_singlecopy)
#a = input('go on...\n')
#print(len(missing_busco_list))
#print(missing_busco_list)
...
##DBG
#  print(missing_busco_list)
#  a = input('press to see the location_dic\n')
#  print(location_dic)
  for entry in missing_busco_list:
      if entry in location_dic:
        for location in location_dic[entry]:
            ...
            sed_call = 'sed -i \'1,3d\' %s' % out_name
